...
internal class TrackingFrame
{
    ...
    public TrackingFrame(IFrame sourceFrame, DownsamplingMethod downsamplingMethod)
    {
        ...
        m_measurements = downsamplingMethod != DownsamplingMethod.LastReceived ? 
                ...
                if (m_measurements.TryGetValue(measurement.Key, out values))
                {
                    if (values is not null && values.Count > 0)
                    {
                        ...
                        MeasurementValueFilterFunction measurementValueFilter =
                            derivedMeasurement.MeasurementValueFilter ?? Measurement.AverageValueFilter;

                        // Add new measurement to tracking collection
                        values.Add(measurement);

                        if (values.Count <= 1)
                            return null; // No change from existing measurement
