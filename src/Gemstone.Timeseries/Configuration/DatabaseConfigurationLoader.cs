...
public class DatabaseConfigurationLoader : ConfigurationLoaderBase, IDisposable
{
    ...
    public override DataSet? Load()
    {
        ...
        Execute(_ =>
        {
            ...
            {
                DataTable? table = LoadTable(entityRow);
    ...
    public override void Augment(DataSet configuration)
    {
        ...
        Execute(database =>
        {
            ...
            Debug.Assert(trackedChanges is not null);
            ...
            if (TrackedChangesAreValid(currentVersion))
            {
                ...
                {
                    trackedTables = database.Connection.RetrieveData("SELECT Name FROM TrackedTable").Select()
    ...
    private DataTable? GetEntities()
    {
        ...
        Execute(database =>
        {
            entities = database.Connection.RetrieveData("SELECT * FROM ConfigurationEntity WHERE Enabled <> 0 ORDER BY LoadOrder");
    ...
    private DataTable? GetTrackedChanges(ulong currentVersion)
    {
        ...
        Execute(database => table = database.Connection.RetrieveData($"SELECT * FROM TrackedChange WHERE ID > {currentVersion}"));
        return table;
    }
    ...
    private void ExtractSecurityContext()
    {
        Execute(database =>
        {
            ...
            // TODO: Implement security context extraction
