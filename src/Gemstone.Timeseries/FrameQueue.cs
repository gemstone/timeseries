...
public class FrameQueue : IDisposable
{
    ...
    /// <summary>
    /// Gets or sets the <see cref="Timeseries.DownsamplingMethod"/> to be used by the <see cref="FrameQueue"/>.
    /// </summary>
    ...
    /// </summary>
    public int Count => m_frameList?.Count ?? 0;
    ...
    public string ExamineQueueState(int expectedMeasurements)
    {
        ...
        try
        {
            ...
            status.AppendLine();
            status.AppendLine($" Ordered frame queue count: {m_frameList?.Count ?? 0:N0}");
            status.AppendLine($"    Frame hash-table count: {m_frameHash?.Count ?? 0:N0}");
    ...
    public void Pop()
    {
        ...
        m_last = m_head;
        m_head = null;
        long publishedTicks = m_last?.Timestamp ?? 0L;
    ...
    public TrackingFrame? GetFrame(long ticks)
    {
        ...
        if (m_frameHash?.TryGetValue(destinationTicks, out TrackingFrame? frame) ?? false)
            return frame;
        ...
        try
        {
            ...
            if (m_frameHash?.TryGetValue(destinationTicks, out frame) ?? false)
                return frame;
