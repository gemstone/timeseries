...
public static class TimeseriesStartupOperations
{
    ...
    private static void ValidateStatistics(AdoDataConnection database)
    {
        ...
        const string StatHistorianCountFormat = "SELECT COUNT(*) FROM Historian WHERE IsStatistic = 1";
        const string StatEngineCountFormat = "SELECT COUNT(*) FROM CustomActionAdapter WHERE AdapterName = 'STATISTIC!SERVICES'";
        const string SystemStatCountFormat = $"SELECT COUNT(*) FROM Statistic WHERE Source = 'System' AND AssemblyName = '{nameof(Gemstone)}.{nameof(Timeseries)}.dll'";
        const string DeviceStatCountFormat = $"SELECT COUNT(*) FROM Statistic WHERE Source = 'Device' AND AssemblyName = '{nameof(Gemstone)}.{nameof(Timeseries)}.dll'";
        const string SubscriberStatCountFormat = $"SELECT COUNT(*) FROM Statistic WHERE Source = 'Subscriber' AND AssemblyName = '{nameof(Gemstone)}.{nameof(Timeseries)}.dll'";
        const string PublisherStatCountFormat = $"SELECT COUNT(*) FROM Statistic WHERE Source = 'Publisher' AND AssemblyName = '{nameof(Gemstone)}.{nameof(Timeseries)}.dll'";
        ...
        const string StatHistorianInsertFormat = "INSERT INTO Historian(Acronym, Name, AssemblyName, TypeName, ConnectionString, IsLocal, Description, LoadOrder, Enabled, IsPrimary, IsStatistic, IsEvents) VALUES('STAT', 'Statistics Archive', 'openHistorian.Adapters.dll', 'openHistorian.Adapters.LocalOutputAdapter', 'WorkingDirectory=Statistics; MaximumArchiveDays=90; DesiredRemainingSpace=0.1; ArchiveCurtailmentInterval=21600; DataChannel={port=38403; interface=::0}', 1, 'Local historian used to archive system statistics', 9999, 1, 0, 1, 0)";
        const string StatEngineInsertFormat = $"INSERT INTO CustomActionAdapter(AdapterName, AssemblyName, TypeName, LoadOrder, Enabled) VALUES('STATISTIC!SERVICES', '{nameof(Gemstone)}.{nameof(Timeseries)}.dll', '{nameof(Gemstone)}.{nameof(Timeseries)}.{nameof(Statistics)}.{nameof(StatisticsEngine)}', 0, 1)";
        const string SystemStatInsertFormat = $"INSERT INTO Statistic(Source, SignalIndex, Name, Description, AssemblyName, TypeName, MethodName, Arguments, Enabled, DataType, DisplayFormat, IsConnectedState, LoadOrder) VALUES('System', {{0}}, '{{1}}', '{{2}}', '{nameof(Gemstone)}.{nameof(Timeseries)}.dll', '{nameof(Gemstone)}.{nameof(Timeseries)}.{nameof(Statistics)}.{nameof(PerformanceStatistics)}', 'GetSystemStatistic_{{3}}', '', 1, '{{4}}', '{{5}}', 0, {{0}})";
        const string DeviceStatInsertFormat = $"INSERT INTO Statistic(Source, SignalIndex, Name, Description, AssemblyName, TypeName, MethodName, Arguments, Enabled, DataType, DisplayFormat, IsConnectedState, LoadOrder) VALUES('Device', {{0}}, '{{1}}', '{{2}}', '{nameof(Gemstone)}.{nameof(Timeseries)}.dll', '{nameof(Gemstone)}.{nameof(Timeseries)}.{nameof(Statistics)}.{nameof(DeviceStatistics)}', 'GetDeviceStatistic_{{3}}', '', 1, '{{4}}', '{{5}}', 0, {{0}})";
        const string SubscriberStatInsertFormat = $"INSERT INTO Statistic(Source, SignalIndex, Name, Description, AssemblyName, TypeName, MethodName, Arguments, Enabled, DataType, DisplayFormat, IsConnectedState, LoadOrder) VALUES('Subscriber', {{0}}, '{{1}}', '{{2}}', 'sttp.gemstone.dll', 'sttp.GatewayStatistics', 'GetSubscriberStatistic_{{3}}', '', 1, '{{4}}', '{{5}}', 0, {{0}})";
        const string PublisherStatInsertFormat = $"INSERT INTO Statistic(Source, SignalIndex, Name, Description, AssemblyName, TypeName, MethodName, Arguments, Enabled, DataType, DisplayFormat, IsConnectedState, LoadOrder) VALUES('Publisher', {{0}}, '{{1}}', '{{2}}', 'sttp.gemstone.dll', 'sttp.GatewayStatistics', 'GetPublisherStatistic_{{3}}', '', 1, '{{4}}', '{{5}}', 0, {{0}})";
        ...
        [
            ...
            /* 02 */ "Processed Measurements",
            /* 03 */ "Total Bytes Received",
            /* 04 */ "Authorized Signal Count",
            /* 05 */ "Unauthorized Signal Count",
            /* 06 */ "Lifetime Measurements",
            /* 07 */ "Lifetime Bytes Received",
            /* 08 */ "Minimum Measurements Per Second",
            /* 09 */ "Maximum Measurements Per Second",
            /* 10 */ "Average Measurements Per Second",
            /* 11 */ "Lifetime Minimum Latency",
            /* 12 */ "Lifetime Maximum Latency",
            /* 13 */ "Lifetime Average Latency",
            /* 14 */ "Up Time",
            /* 15 */ "TLS Secured Channel"
        ];
        ...
        [
            ...
            /* 02 */ "Number of processed measurements reported by the subscriber during last reporting interval.",
            /* 03 */ "Number of bytes received from subscriber during last reporting interval.",
            /* 04 */ "Number of signals authorized to the subscriber by the publisher.",
            /* 05 */ "Number of signals denied to the subscriber by the publisher.",
            /* 06 */ "Number of processed measurements reported by the subscriber during the lifetime of the subscriber.",
            /* 07 */ "Number of bytes received from subscriber during the lifetime of the subscriber.",
            /* 08 */ "The minimum number of measurements received per second during the last reporting interval.",
            /* 09 */ "The maximum number of measurements received per second during the last reporting interval.",
            /* 10 */ "The average number of measurements received per second during the last reporting interval.",
            /* 11 */ "Minimum latency from output stream, in milliseconds, during the lifetime of the subscriber.",
            /* 12 */ "Maximum latency from output stream, in milliseconds, during the lifetime of the subscriber.",
            /* 13 */ "Average latency from output stream, in milliseconds, during the lifetime of the subscriber.",
            /* 14 */ "Total number of seconds subscriber has been running.",
            /* 15 */ "Boolean value representing if subscriber command channel has transport layer security enabled"
        ];

        string[] SubscriberStatMethodSuffix =
        [
            /* 01 */ "Connected",
            /* 02 */ "ProcessedMeasurements",
            /* 03 */ "TotalBytesReceived",
            /* 04 */ "AuthorizedCount",
            /* 05 */ "UnauthorizedCount",
            /* 06 */ "LifetimeMeasurements",
            /* 07 */ "LifetimeBytesReceived",
            /* 08 */ "MinimumMeasurementsPerSecond",
            /* 09 */ "MaximumMeasurementsPerSecond",
            /* 10 */ "AverageMeasurementsPerSecond",
            /* 11 */ "LifetimeMinimumLatency",
            /* 12 */ "LifetimeMaximumLatency",
            /* 13 */ "LifetimeAverageLatency",
            /* 14 */ "UpTime",
            /* 15 */ "TLSSecuredChannel"
        ];
        ...
        [
            ...
            /* 13 */ "Buffer Block Retransmissions",
            /* 14 */ "Up Time",
            /* 15 */ "TLS Secured Channel"
        ];
        ...
        [
            ...
            /* 13 */ "Total number of buffer block retransmissions during the last reporting interval.",
            /* 14 */ "Total number of seconds publisher has been running.",
            /* 15 */ "Boolean value representing if publisher command channel has transport layer security enabled"
        ];
